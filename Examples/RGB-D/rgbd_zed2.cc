...
void LoadImages(const string& strAssociationFolder,
                vector<string>& vstrImageFilenamesRGB,
                vector<string>& vstrImageFilenamesD,
                vector<double>& vTimestamps) {
  ifstream fAssociation, fDepth;
  string strPathTimesLeft = strAssociationFolder + "/left.txt";
  string strPathTimesDepth = strAssociationFolder + "/depth.txt";
  fAssociation.open(strPathTimesLeft.c_str());
  while (!fAssociation.eof()) {
    ...
    if (!s.empty()) {
      ...
      ss >> sRGB;
      vstrImageFilenamesRGB.push_back(strAssociationFolder + "/" + sRGB);
    }
  }
  fAssociation.close();
  fDepth.open(strPathTimesDepth.c_str());
  while (!fDepth.eof()) {
    string s;
    getline(fDepth, s);
    if (!s.empty()) {
      ...
      ss >> t;
      ss >> sD;
      vstrImageFilenamesD.push_back(strAssociationFolder + "/" + sD);
    }
  }
  fDepth.close();
}
