...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer,
    ...
    : mState(NO_IMAGES_YET),
      ...
      mbMapUpdated(false),
      mpOdomPreintegratedFromLastKF(nullptr),
      mbVO(false),
...
#ifdef REGISTER_TIMES
double calcAverage(const vector<double>& v_times) {
  double accum = 0;
  for (double value : v_times) {
    accum += value;
  }

  return accum / (double)v_times.size();
}
...
double calcDeviation(const vector<double>& v_times, double average) {
  ...
  return sqrt(accum / (double)v_times.size());
}

double calcAverage(const vector<int>& v_values) {
  double accum = 0;
  int total = 0;
...
double calcDeviation(const vector<int>& v_values, double average) {
  double accum = 0;
  int total = 0;
  for (double value : v_values) {
    if (value == 0) continue;
...
void Tracking::PrintTimeStats() {
  ...
  for (size_t i = 1; i < vpMaps.size(); ++i) {
...
void Tracking::NewParameterLoader(Settings* settings) {
  ...
  int fIniThFAST = static_cast<int>(settings->initThFAST());
  int fMinThFAST = static_cast<int>(settings->minThFAST());
  ...
  // WOdometry parameters
  mbUseOdom = settings->bUseOdom();
  mTbo = settings->Tbo();
  mTco = settings->Tbc().inverse() * settings->Tbo();
  mOdomBias << settings->noiseX(), settings->noiseY(), settings->noiseRotZ_();
  if (mbUseOdom)
    mpOdomPreintegratedFromLastKF = new ODOM::Preintegrated(mOdomBias, mTbo);
  else
    mpOdomPreintegratedFromLastKF = nullptr;
}
...
bool Tracking::ParseCamParamFile(cv::FileStorage& fSettings) {
  ...
  if (sCameraName == "PinHole") {
    ...
    if (!node.empty() && node.isReal()) {
      mDistCoef.at<float>(0) = static_cast<float>(node.real());
    } else {
    ...
    if (!node.empty() && node.isReal()) {
      mDistCoef.at<float>(1) = static_cast<float>(node.real());
    } else {
    ...
    if (!node.empty() && node.isReal()) {
      mDistCoef.at<float>(2) = static_cast<float>(node.real());
    } else {
    ...
    if (!node.empty() && node.isReal()) {
      mDistCoef.at<float>(3) = static_cast<float>(node.real());
    } else {
    ...
    if (!node.empty() && node.isReal()) {
      mDistCoef.resize(5);
      mDistCoef.at<float>(4) = static_cast<float>(node.real());
    }

    node = fSettings["Camera.imageScale"];
    if (!node.empty() && node.isReal()) {
      mImageScale = static_cast<float>(node.real());
    }
    ...
    node = fSettings["Camera.k1"];
    if (!node.empty() && node.isReal()) {
      k1 = static_cast<float>(node.real());
    } else {
      std::cerr << "*Camera.k1 parameter doesn't exist or is not a real number*"
                << std::endl;
      b_miss_params = true;
    ...
    node = fSettings["Camera.k2"];
    if (!node.empty() && node.isReal()) {
      k2 = static_cast<float>(node.real());
    } else {
      std::cerr << "*Camera.k2 parameter doesn't exist or is not a real number*"
                << std::endl;
      b_miss_params = true;
    ...
    node = fSettings["Camera.k3"];
    if (!node.empty() && node.isReal()) {
      k3 = static_cast<float>(node.real());
    } else {
    ...
    if (!node.empty() && node.isReal()) {
      k4 = static_cast<float>(node.real());
    } else {
    ...
    if (mSensor == System::STEREO || mSensor == System::IMU_STEREO ||
      ...
      // Right camera
      // Camera calibration parameters
      cv::FileNode node_ = fSettings["Camera2.fx"];
      if (!node_.empty() && node_.isReal()) {
        fx = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera2.fy"];
      if (!node_.empty() && node_.isReal()) {
        fy = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera2.cx"];
      if (!node_.empty() && node_.isReal()) {
        cx = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera2.cy"];
      if (!node_.empty() && node_.isReal()) {
        cy = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      // Distortion parameters
      node_ = fSettings["Camera2.k1"];
      if (!node_.empty() && node_.isReal()) {
        k1 = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera2.k2"];
      if (!node_.empty() && node_.isReal()) {
        k2 = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera2.k3"];
      if (!node_.empty() && node_.isReal()) {
        k3 = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera2.k4"];
      if (!node_.empty() && node_.isReal()) {
        k4 = static_cast<float>(node_.real());
      } else {
        std::cerr
      ...
      node_ = fSettings["Camera.lappingBegin"];
      if (!node_.empty() && node_.isInt()) {
        leftLappingBegin = node_.operator int();
      } else {
        std::cout << "WARNING: Camera.lappingBegin not correctly defined"
                  << std::endl;
      }
      node_ = fSettings["Camera.lappingEnd"];
      if (!node_.empty() && node_.isInt()) {
        leftLappingEnd = node_.operator int();
      } else {
        std::cout << "WARNING: Camera.lappingEnd not correctly defined"
                  << std::endl;
      }
      node_ = fSettings["Camera2.lappingBegin"];
      if (!node_.empty() && node_.isInt()) {
        rightLappingBegin = node_.operator int();
      } else {
        std::cout << "WARNING: Camera2.lappingBegin not correctly defined"
                  << std::endl;
      }
      node_ = fSettings["Camera2.lappingEnd"];
      if (!node_.empty() && node_.isInt()) {
        rightLappingEnd = node_.operator int();
      } else {
  ...
  if (mSensor == System::STEREO || mSensor == System::RGBD ||
    ...
    if (!node.empty() && node.isReal()) {
      mThDepth = static_cast<float>(node.real());
...
bool Tracking::ParseORBParamFile(cv::FileStorage& fSettings) {
  ...
  if (!node.empty() && node.isReal()) {
    fScaleFactor = static_cast<float>(node.real());
  } else {
...
bool Tracking::GetStepByStep() const { return bStepByStep; }
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat& im,
  ...
  lastID = static_cast<int>(mCurrentFrame.mnId);
  Track();
...
void Tracking::PreintegrateIMU() {
  ...
  const size_t n = mvImuFromLastFrame.size() - 1;
  if (n == 0) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(
        acc, angVel, static_cast<float>(tstep));
...
/**
 *   \brief Perform online interpolation of measurements
 *   \param mvOdomFromLastFrame Vector of measurements from last frame
 *   \return Success?
 *
 **/
...
void Tracking::PreintegrateOdom() {
  ...
  if (mlQueueOdomData.empty()) {
    Verbose::PrintMess("Not Odom data in mlQueueImuData!!",
                       Verbose::VERBOSITY_NORMAL);
    return;
  }
  ...
  while (true) {
    bool bSleep = false;
    {
      ...
      if (!mlQueueOdomData.empty()) {
        ODOM::Meas* m = &mlQueueOdomData.front();
        if (m->t < mCurrentFrame.mpPrevFrame->mTimeStamp - mImuPer) {
          mlQueueOdomData.pop_front();
        } else if (m->t < mCurrentFrame.mTimeStamp - mImuPer) {
          mvOdomFromLastFrame.push_back(*m);
          mlQueueOdomData.pop_front();
        } else {
          mvOdomFromLastFrame.push_back(*m);
          break;
  ...
  const size_t n = mvOdomFromLastFrame.size() - 1;
  if (n == 0) {
    cout << "Empty WOdometry measurements vector!!!\n";
    return;
  }

  // Create Preintegrate from last frame pointer here!
  auto* pOdomPreintegratedFromLastFrame =
      new ODOM::Preintegrated(mOdomBias, mTbo);

  for (size_t i = 0; i < n; ++i) {
    double tstep;
    if (i == 0 && i < (n - 1)) {
      // begin
      double tab = mvOdomFromLastFrame[i + 1].t - mvOdomFromLastFrame[i].t;
      double tini =
          mvOdomFromLastFrame[i].t - mCurrentFrame.mpPrevFrame->mTimeStamp;
      Eigen::Vector2f TransPrevFrame;
      long double AnglePrevFrame;
      if (tab != 0) {
        TransPrevFrame = mvOdomFromLastFrame[i].meas.translation() -
                         (mvOdomFromLastFrame[i + 1].meas.translation() -
                          mvOdomFromLastFrame[i].meas.translation()) *
                             (tini / tab);
        AnglePrevFrame = mvOdomFromLastFrame[i].meas.so2().log() -
                         (mvOdomFromLastFrame[i + 1].meas.so2().log() -
                          mvOdomFromLastFrame[i].meas.so2().log()) *
                             (tini / tab);
      } else {
        // TODO(Vuong): Fix this! Why do 2 meas have same timestamp?
        TransPrevFrame = mvOdomFromLastFrame[i].meas.translation();
        AnglePrevFrame = mvOdomFromLastFrame[i].meas.so2().log();
      }

      Sophus::SE2f PrevFrameMeas((float)AnglePrevFrame, TransPrevFrame);

      tstep =
          mvOdomFromLastFrame[i + 1].t - mCurrentFrame.mpPrevFrame->mTimeStamp;
      // 2 preintegrate
      // Prev-frame -> 0th
      mpOdomPreintegratedFromLastKF->IntegratedNewMeasurement(
          PrevFrameMeas, mvOdomFromLastFrame[i].meas);
      pOdomPreintegratedFromLastFrame->IntegratedNewMeasurement(
          PrevFrameMeas, mvOdomFromLastFrame[i].meas);
      // (i)th -> (i+1)th
      mpOdomPreintegratedFromLastKF->IntegratedNewMeasurement(
          mvOdomFromLastFrame[i].meas, mvOdomFromLastFrame[i + 1].meas);
      pOdomPreintegratedFromLastFrame->IntegratedNewMeasurement(
          mvOdomFromLastFrame[i].meas, mvOdomFromLastFrame[i + 1].meas);
    } else if (i < (n - 1)) {
      // middle
      ...
      // end
      double tab = mvOdomFromLastFrame[i + 1].t - mvOdomFromLastFrame[i].t;
      double tend = mvOdomFromLastFrame[i + 1].t - mCurrentFrame.mTimeStamp;
      Eigen::Vector2f TransCurrFrame =
          mvOdomFromLastFrame[i + 1].meas.translation() -
          (mvOdomFromLastFrame[i + 1].meas.translation() -
           mvOdomFromLastFrame[i].meas.translation()) *
              (tend / tab);
      auto AngleCurrFrame = (mvOdomFromLastFrame[i + 1].meas.so2().log() -
                             (mvOdomFromLastFrame[i + 1].meas.so2().log() -
                              mvOdomFromLastFrame[i].meas.so2().log()) *
                                 (tend / tab));
      Sophus::SE2f CurrFrameMeas((float)AngleCurrFrame, TransCurrFrame);
      // 2 preintegrate
      // (i)th -> (i+1)th
      mpOdomPreintegratedFromLastKF->IntegratedNewMeasurement(
          mvOdomFromLastFrame[i].meas, mvOdomFromLastFrame[i + 1].meas);
...
void Tracking::Track() {
  ...
  if (mState == NOT_INITIALIZED) {
    ...
    if (!mbOnlyTracking) {
      ...
      if (mState == OK) {
        ...
        if (mState == RECENTLY_LOST) {
          ...
          if (mpLastKeyFrame) mpLastKeyFrame = static_cast<KeyFrame*>(nullptr);
    ...
    if ((mCurrentFrame.mnId < (mnLastRelocFrameId + mnFramesToResetIMU)) &&
        (mCurrentFrame.mnId > (size_t)mnFramesToResetIMU) &&
...
void Tracking::CreateInitialMapMonocular() {
  ...
  for (size_t i = 0; i < mvIniMatches.size(); i++) {
    ...
    pMP->AddObservation(pKFini, static_cast<int>(i));
  ...
  initID = static_cast<int>(pKFcur->mnId);
}
...
void Tracking::CreateMapInAtlas() {
  ...
  if (mpReferenceKF) mpReferenceKF = static_cast<KeyFrame*>(nullptr);
...
void Tracking::UpdateLastFrame() {
  ...
  for (auto& j : vDepthIdx) {
    ...
    if (!pMP || pMP->Observations() < 1) bCreateNew = true;
    ...
    if (j.first > mThDepth && nPoints > 100) break;
  }
}
...
bool Tracking::TrackWithMotionModel() {
  ...
  if (nmatches < 20) {
    ...
    nmatches = matcher.SearchByProjection(
        mCurrentFrame, mLastFrame, static_cast<float>(2 * th),
        mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
  ...
  if (!mpAtlas->isImuInitialized())
    ...
    if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU) {
      ...
      if (!mbMapUpdated) {
        ...
        WOdometryOptimization::PoseInertialOptimizationLastFrame(
            &mCurrentFrame, mpPlanarConstraint);
      } else {
        Verbose::PrintMess("TLM: PoseInertialOptimizationLastKeyFrame ",
                           Verbose::VERBOSITY_DEBUG);
        //        Optimizer::PoseInertialOptimizationLastKeyFrame(&mCurrentFrame);
        WOdometryOptimization::PoseInertialOptimizationLastKeyFrame(
            &mCurrentFrame, mpPlanarConstraint);
      }
    }
  }
...
bool Tracking::NeedNewKeyFrame() {
  ...
  const int nKFs = static_cast<int>(mpAtlas->KeyFramesInMap());
...
void Tracking::SearchLocalPoints() {
  ...
  size_t nN = mCurrentFrame.mvpMapPoints.size();
  tbb::parallel_for(
      tbb::blocked_range<size_t>(0, nN), [&](tbb::blocked_range<size_t> rnN) {
        for (size_t i = rnN.begin(); i < rnN.end(); i++) {
          MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
          if (pMP) {
            if (pMP->isBad())
              mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(nullptr);
...
void Tracking::ResetActiveMap(bool bLocMap) {
  ...
  for (Map* pMap_ : mpAtlas->GetAllMaps()) {
    if (!pMap_->GetAllKeyFrames().empty()) {
      if (index > pMap_->GetLowerKFID()) index = pMap_->GetLowerKFID();
    }
  }
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias& b,
  ...
  mnFirstImuFrameId = static_cast<int>(mCurrentFrame.mnId);
}

void Tracking::NewDataset() { mnNumDataset++; }

int Tracking::GetNumberDataset() const { return mnNumDataset; }

int Tracking::GetMatchesInliers() const { return mnMatchesInliers; }

void Tracking::SaveSubTrajectory(const string& strNameFile_frames,
...
float Tracking::GetImageScale() const { return mImageScale; }
void Tracking::GrabOdomData(const ODOM::Meas& odomMeasurement) {
  unique_lock<mutex> lock(mMutexOdomQueue);
  mlQueueOdomData.push_back(odomMeasurement);
}
void Tracking::UpdateTbo(Sophus::SE3f& NewTbo) { mTbo = NewTbo; }
...
void Tracking::SetPlanarConstraints(PlanarConstraint* pPlanarConstraint) {
  mpPlanarConstraint = pPlanarConstraint;
}
bool Tracking::PredictStateOdom() {
  ...
  if (mbMapUpdated && mpLastKeyFrame) {
    const Sophus::SE3f Twc1 = mpLastKeyFrame->GetPose().inverse();
    Sophus::SE3f Two1 = Twc1 * mTco;
    Eigen::Rotation2Df Ro12_2D(mpOdomPreintegratedFromLastKF->Delta.z());
    Eigen::Matrix3f dRo12;
    dRo12.setIdentity();
    dRo12.block<2, 2>(0, 0) = Ro12_2D.toRotationMatrix();
    Sophus::SE3f dTo12 = Sophus::SE3f(
        Eigen::Quaternionf(dRo12).normalized(),
        Eigen::Vector3f(mpOdomPreintegratedFromLastKF->Delta.x(),
                        mpOdomPreintegratedFromLastKF->Delta.y(), 0));
    Sophus::SE3f Two2 = Two1 * dTo12;
    Sophus::SE3f Twc2 = Two2 * mTco.inverse();
    mCurrentFrame.SetPose(Twc2.inverse());
    return true;
  } else if (!mbMapUpdated) {
    const Sophus::SE3f Twc1 = mLastFrame.GetPose().inverse();
    Sophus::SE3f Two1 = Twc1 * mTco;
    Eigen::Rotation2Df Ro12_2D(
        mCurrentFrame.mpOdomPreintegratedFrame->Delta.z());
    Eigen::Matrix3f dRo12;
    dRo12.setIdentity();
    dRo12.block<2, 2>(0, 0) = Ro12_2D.toRotationMatrix();
    Sophus::SE3f dTo12 = Sophus::SE3f(
        Eigen::Quaternionf(dRo12).normalized(),
        Eigen::Vector3f(mCurrentFrame.mpOdomPreintegratedFrame->Delta.x(),
                        mCurrentFrame.mpOdomPreintegratedFrame->Delta.y(), 0));
    Sophus::SE3f Two2 = Two1 * dTo12;
    Sophus::SE3f Twc2 = Two2 * mTco.inverse();
    mCurrentFrame.SetPose(Twc2.inverse());
    return true;
