...
void Preintegrated::IntegratedNewMeasurement(Sophus::SE2f& PrevMeas,
  ...
  Eigen::Matrix2f Phi_ik = Eigen::Rotation2Df(Delta.z()).toRotationMatrix();
  //  Meas = Meas * delta;
...
{}
void Preintegrated::MergePrevious(ODOM::Preintegrated* pPrev) {
  if (pPrev == this) return;
  Delta.head(2) += Eigen::Rotation2Df(Delta.z()) * pPrev->Delta.head(2);
  Delta.z() += pPrev->Delta.z();
  Cov1 *= pPrev->Cov1;
}
}  // namespace ODOM
}  // namespace ORB_SLAM3